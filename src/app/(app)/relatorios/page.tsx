    ...
    <div className="flex flex-col gap-8">
      <div className="flex items-center justify-between">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-page-subtitle">Para onde o dinheiro foi e de onde veio, por categoria.</p>
        </div>
        <MonthSelector
          label={monthLabel(year, monthIndex)}
          prevHref={`/relatorios?month=${monthParam(prev.year, prev.monthIndex)}`}
          nextHref={`/relatorios?month=${monthParam(next.year, next.monthIndex)}`}
        />
      </div>
      ...
      <div className="grid grid-cols-2 gap-5">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="text-card-title">Receitas por categoria</CardTitle>
          ...
          <CardContent>
            {incomeRanking.length === 0 ? (
              <p className="text-sm text-(--text-muted)">Nenhuma receita neste mês.</p>
            ) : (
